...
function FAQFeedbackTab({ dark }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        console.warn("FAQ feedback fetch:", e);
      } finally {
        setLoading(false);
      }
    })();
  ...
  const grouped = useMemo(() => {
    const map = {};
    votes.forEach(v => {
      if (!map[v.faqId]) map[v.faqId] = { faqId: v.faqId, cat: v.cat || "?", q: v.q || v.faqId, up: 0, down: 0 };
  ...
  const CAT_COLOR = { about: NAVY, privacy: IND_GREEN, schemes: SAFFRON, ai: VIOLET, account: "#0284C7" };
  ...
  if (loading) return (
    <div style={{ display:"flex", alignItems:"center", justifyContent:"center", padding:60, color:th.textSub, fontSize:12, fontFamily:bf }}>
      Loading feedback…
    </div>
  );
  ...
  if (total === 0) return (
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:60, gap:8 }}>
      ...
      <div style={{ fontSize:13, fontWeight:700, color:th.text, fontFamily:bf }}>No feedback yet</div>
      <div style={{ fontSize:11, color:th.textSub, fontFamily:bf, textAlign:"center", maxWidth:260 }}>
        FAQ 👍/👎 votes will appear here once users start rating answers.
      </div>
    </div>
  );
  ...
  return (
    <div style={{ padding:"14px 14px 28px", display:"flex", flexDirection:"column", gap:14 }}>
      ...
      <div style={{ display:"flex", gap:8 }}>
        {[
          { label:"Responses",      value: total,       color: NAVY    },
          { label:"Helpful 👍",     value: totalUp,     color: IND_GREEN },
          { label:"Not helpful 👎", value: totalDown,   color: "#E53E3E" },
          { label:"Satisfaction",   value: `${overallPct}%`,
            color: overallPct >= 70 ? IND_GREEN : overallPct >= 40 ? SAFFRON : "#E53E3E" },
        ].map(({ label, value, color }) => (
          <div key={label} style={{
            flex:1, background:th.card, borderRadius:12, textAlign:"center",
            border:`1.5px solid ${dark ? "rgba(255,255,255,0.07)" : "rgba(0,0,0,0.06)"}`,
            padding:"10px 6px 9px",
          }}>
            <div style={{ fontSize:18, fontWeight:800, color, fontFamily:bf }}>{value}</div>
            <div style={{ fontSize:8.5, color:th.textSub, fontFamily:bf, marginTop:2, lineHeight:1.3 }}>{label}</div>
          </div>
        ))}
      </div>

      {/* ── Category breakdown ── */}
      <div style={{ background:th.card, borderRadius:14, padding:"12px 13px",
        border:`1.5px solid ${dark ? "rgba(255,255,255,0.07)" : "rgba(0,0,0,0.06)"}` }}>
        <div style={{ fontSize:10, fontWeight:800, color:th.textSub, letterSpacing:0.7, textTransform:"uppercase", marginBottom:10, fontFamily:bf }}>
          By Category
        </div>
        <div style={{ display:"flex", flexDirection:"column", gap:9 }}>
          {Object.entries(catStats).map(([cat, { up, down }]) => {
            const pct = Math.round((up / (up + down)) * 100);
            const cc  = CAT_COLOR[cat] || NAVY;
            return (
              <div key={cat} style={{ display:"flex", alignItems:"center", gap:9 }}>
                <div style={{ width:58, fontSize:10, fontWeight:700, color:cc, fontFamily:bf, textTransform:"capitalize" }}>{cat}</div>
                <div style={{ flex:1, height:6, borderRadius:3, background:dark ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.07)", overflow:"hidden" }}>
                  <div style={{ width:`${pct}%`, height:"100%", borderRadius:3, background:cc }} />
                </div>
                <div style={{ fontSize:10, fontWeight:800, width:30, textAlign:"right", fontFamily:bf,
                  color: pct >= 70 ? IND_GREEN : pct >= 40 ? SAFFRON : "#E53E3E" }}>{pct}%</div>
                <div style={{ fontSize:9, color:th.textSub, fontFamily:bf, whiteSpace:"nowrap" }}>👍{up} 👎{down}</div>
              </div>
            );
          })}
        </div>
      ...
      {/* ── Per-question list ── */}
      <div style={{ background:th.card, borderRadius:14, overflow:"hidden",
        border:`1.5px solid ${dark ? "rgba(255,255,255,0.07)" : "rgba(0,0,0,0.06)"}` }}>

        {/* Table header + sort */}
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between",
          padding:"11px 13px", borderBottom:`1px solid ${dark ? "rgba(255,255,255,0.06)" : "rgba(0,0,0,0.06)"}` }}>
          <div style={{ fontSize:10, fontWeight:800, color:th.textSub, letterSpacing:0.7, textTransform:"uppercase", fontFamily:bf }}>
            All Questions · {sorted.length}
          </div>
          <div style={{ display:"flex", gap:5 }}>
            {[["worst","⬇ Worst"], ["best","⬆ Best"], ["popular","🔥 Most voted"]].map(([id, label]) => (
              <button key={id} type="button" onClick={() => setSortBy(id)} style={{
                fontSize:9, fontWeight:700, fontFamily:bf,
                padding:"3px 7px", borderRadius:20, cursor:"pointer", margin:0,
                background: sortBy === id ? NAVY : "transparent",
                color:      sortBy === id ? "#fff" : th.textSub,
                border:`1px solid ${sortBy === id ? NAVY : (dark ? "rgba(255,255,255,0.12)" : "rgba(0,0,0,0.10)")}`,
              }}>{label}</button>
            ))}
          </div>
        </div>
        ...
        {sorted.map((row, i) => {
          ...
          const cc   = CAT_COLOR[row.cat] || NAVY;
          const isLow = pct < 50;
          return (
            <div key={row.faqId} style={{
              padding:"10px 13px",
              background: isLow ? (dark ? "rgba(229,62,62,0.04)" : "rgba(229,62,62,0.025)") : "transparent",
              ...
              <div style={{ display:"flex", alignItems:"flex-start", gap:7, marginBottom:5 }}>
                <div style={{
                  fontSize:8.5, fontWeight:800, color:cc, background:`${cc}18`,
                  border:`1px solid ${cc}30`, borderRadius:5, padding:"1px 5px",
                  flexShrink:0, marginTop:1, fontFamily:bf, textTransform:"capitalize",
                }}>{row.cat}</div>
                <div style={{ fontSize:11.5, fontWeight:600, color:th.text, fontFamily:bf, lineHeight:1.4, flex:1 }}>
                  {row.q}
                </div>
                <div style={{ fontSize:11, fontWeight:800, flexShrink:0, fontFamily:bf,
                  color: pct >= 70 ? IND_GREEN : pct >= 40 ? SAFFRON : "#E53E3E" }}>
                  {pct}%
                </div>
              </div>
              <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                <div style={{ flex:1, height:4, borderRadius:2, background:dark ? "rgba(255,255,255,0.07)" : "rgba(0,0,0,0.06)", overflow:"hidden" }}>
                  <div style={{ width:`${pct}%`, height:"100%", borderRadius:2,
                    background: pct >= 70 ? IND_GREEN : pct >= 40 ? SAFFRON : "#E53E3E" }} />
                </div>
